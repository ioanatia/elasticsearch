...
public class ToText extends AbstractConvertFunction implements EvaluatorMapper, ConfigurationAware {
    ...
    public static final FunctionDefinition DEFINITION = FunctionDefinition.def(ToText.class).unaryConfig(ToText::new).name("to_text");
    ...
    private static final Map<DataType, BuildFactory> STATIC_EVALUATORS = Map.ofEntries(
        ...
        Map.entry(BOOLEAN, ToTextFromBooleanEvaluator.Factory::new),
        Map.entry(IP, ToTextFromIPEvaluator.Factory::new),
        Map.entry(DENSE_VECTOR, ToTextFromFloatEvaluator.Factory::new),
        Map.entry(DOUBLE, ToTextFromDoubleEvaluator.Factory::new),
        Map.entry(LONG, ToTextFromLongEvaluator.Factory::new),
        Map.entry(INTEGER, ToTextFromIntEvaluator.Factory::new),
        Map.entry(TEXT, (source, fieldEval) -> fieldEval),
        Map.entry(VERSION, ToTextFromVersionEvaluator.Factory::new),
        Map.entry(UNSIGNED_LONG, ToTextFromUnsignedLongEvaluator.Factory::new),
        Map.entry(GEO_POINT, ToTextFromGeoPointEvaluator.Factory::new),
        Map.entry(CARTESIAN_POINT, ToTextFromCartesianPointEvaluator.Factory::new),
        Map.entry(CARTESIAN_SHAPE, ToTextFromCartesianShapeEvaluator.Factory::new),
        Map.entry(GEO_SHAPE, ToTextFromGeoShapeEvaluator.Factory::new),
        Map.entry(GEOHASH, (source, fieldEval) -> new ToTextFromGeoGridEvaluator.Factory(source, fieldEval, GEOHASH)),
        Map.entry(GEOTILE, (source, fieldEval) -> new ToTextFromGeoGridEvaluator.Factory(source, fieldEval, GEOTILE)),
        Map.entry(GEOHEX, (source, fieldEval) -> new ToTextFromGeoGridEvaluator.Factory(source, fieldEval, GEOHEX)),
        // Map.entry(AGGREGATE_METRIC_DOUBLE, ToTextFromAggregateMetricDoubleEvaluator.Factory::new),
        Map.entry(HISTOGRAM, ToTextFromHistogramEvaluator.Factory::new),
        Map.entry(EXPONENTIAL_HISTOGRAM, ToTextFromExponentialHistogramEvaluator.Factory::new),
    ...
    protected Map<DataType, BuildFactory> factories() {
        return new HashMap<>(STATIC_EVALUATORS);
    }
    ...
    public Expression replaceChildren(List<Expression> newChildren) {
        return new ToText(source(), newChildren.get(0), configuration);
    }

    @Override
    protected NodeInfo<? extends Expression> info() {
        return NodeInfo.create(this, ToText::new, field(), configuration);
    }
    ...
    @Override
    public ToText withConfiguration(Configuration configuration) {
        return new ToText(source(), field(), configuration);
    }
